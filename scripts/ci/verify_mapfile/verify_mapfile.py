...
_mapfile_csv_fields = ('Family-model', 'Version', 'Filename', 'EventType', 'Core Type',
                       'Native Model ID', 'Core Role Name')
        ...
        for row in mapfile_rows:
            ...
            if key in event_and_core_types:
                ...
                if event_type == 'hybridcore':
                    msg = (f'Family-model {model} includes duplicate entry for'
                           f' EventType={event_type} and Core Type={core_type}.')
                raise RuntimeError(msg)

            event_and_core_types[key] = 1


def verify_mapfile_model_unique_versions(perfmon_repo_path: Path):
    """Per Family-Model verify only one version is used across all entries.
    ...
    for model in get_unique_mapfile_models(mapfile_data):
        ...
        event_file_versions = [x['Version'] for x in mapfile_data if x['Family-model'] == model]
...
if __name__ == '__main__':
    ...
    verifications = [
        ...
        verify_mapfile_model_unique_versions,
    ]
